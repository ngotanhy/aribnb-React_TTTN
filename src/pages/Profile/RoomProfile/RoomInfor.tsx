...
import { BsFillForwardFill, BsFillGeoAltFill, BsJoystick } from "react-icons/bs";
      ...
      <div className="grid grid-cols-5 mt-8 cursor-default">
        ...
        <div className="relative col-span-2 mt-3 sm:mt-0 sm:col-span-3 sm:pl-4 flex flex-col justify-between">
               <div className="Infor_room">
                ...
                <span className="text-rose-500 text-xl"><BsFillGeoAltFill/></span>
                  <p className="text-base text-rose-600 font-semibold w-11/12 ml-2">{data?.tenPhong}</p>
                </div>
                <p className="m-0 mt-4 text-sm font-bold ml-10">{mainbooking.soLuongKhach} khách - {data?.phongNgu} phòng ngủ - {data?.phongTam} phòng tắm</p>
                <p className="flex justify-start w-9/12 mt-4 ml-10">
                  <p className="mr-4">{moment(mainbooking.ngayDen).format(formatDay)}</p>
                  <span className="mr-4"><BsFillForwardFill /></span>
                  <p>{moment(mainbooking.ngayDi).format(formatDay)}</p>
                </p>
                <p className="flex text-sm font-extrabold justify-end w-10/12 mt-4"> Giá tiền: {data?.giaTien}$ / ngày </p>
              </div>
        </div>
      </div>
  );
